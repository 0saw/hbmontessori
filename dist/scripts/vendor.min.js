...
function simpleCloneInputData(input){
 var pointers=[];var i=0;while(i<input.pointers.length){pointers[i]={clientX:round(input.pointers[i].clientX),clientY:round(input.pointers[i].clientY)};i++;}
...
i++;}
return true;},recognize:function(inputData){
 var inputDataClone=assign({},inputData);if(!boolOrFn(this.options.enable,[this,inputDataClone])){this.reset();this.state=STATE_FAILED;return;} 
...
inherit(PressRecognizer,Recognizer,{defaults:{event:'press',pointers:1,time:251, threshold:9
},getTouchAction:function(){return[TOUCH_ACTION_AUTO];},process:function(input){var options=this.options;var validPointers=input.pointers.length===options.pointers;var validMovement=input.distance<options.threshold;var validTime=input.deltaTime>options.time;this._input=input;
 if(!validMovement||!validPointers||(input.eventType&(INPUT_END|INPUT_CANCEL)&&!validTime)){this.reset();}else if(input.eventType&INPUT_START){this.reset();this._timer=setTimeoutContext(function(){this.state=STATE_RECOGNIZED;this.tryEmit();},options.time,this);}else if(input.eventType&INPUT_END){return STATE_RECOGNIZED;}
...
inherit(TapRecognizer,Recognizer,{defaults:{event:'tap',pointers:1,taps:1,interval:300, time:250,threshold:9, posThreshold:10
...
this._input=input;var tapCount=this.count%options.taps;if(tapCount===0){
 if(!this.hasRequireFailures()){return STATE_RECOGNIZED;}else{this._timer=setTimeoutContext(function(){this.state=STATE_RECOGNIZED;this.tryEmit();},options.interval,this);return STATE_BEGAN;}}}
...
this.touchAction.preventDefaults(inputData);var recognizer;var recognizers=this.recognizers;
...
var remcss=document.createElement('style');remcss.setAttribute('type','text/css');remcss.id='remReplace';document.getElementsByTagName('head')[0].appendChild(remcss); if(remcss.styleSheet){remcss.styleSheet.cssText=rules;}else{remcss.appendChild(document.createTextNode(rules));}},xhr=function(url,callback){ try{



var xhr=window.ActiveXObject?(new ActiveXObject('Microsoft.XMLHTTP')||new ActiveXObject('Msxml2.XMLHTTP')):new XMLHttpRequest();xhr.open('GET',url,true);xhr.onreadystatechange=function(){if(xhr.readyState===4){callback(xhr,url);}
};xhr.send(null);}catch(e){if(window.XDomainRequest){var xdr=new XDomainRequest();xdr.open('get',url);xdr.onload=function(){callback(xdr,url);};xdr.onerror=function(){return false;};xdr.send();}}},removeMediaQueries=function(css){ if(!window.matchMedia&&!window.msMatchMedia){css=css.replace(/@media[\s\S]*?\}\s*\}/g,"");}
return css;};if(!cssremunit()){ var rules='', links=isStyleSheet(), importLinks=[], found=[], foundProps=[], preCSS=[], CSSLinks=[], css=[], fontSize='';

 fontSize=(function(){var doc=document,docElement=doc.documentElement,body=doc.body||doc.createElement('body'),isFakeBody=!doc.body,div=doc.createElement('div'),currentSize=body.style.fontSize,size;if(isFakeBody){docElement.appendChild(body);}
...
return size;}());processLinks();}
})(window);(function(global,factory){"use strict";if(typeof module==="object"&&typeof module.exports==="object"){module.exports=global.document?factory(global,true):function(w){if(!w.document){throw new Error("jQuery requires a window with a document");}
...
var
version="3.1.0", jQuery=function(selector,context){return new jQuery.fn.init(selector,context);},
...
return high!==high||escapedWhitespace?escaped:high<0? String.fromCharCode(high+0x10000):String.fromCharCode(high>>10|0xD800,high&0x3FF|0xDC00);},
 rcssescape=/([\0-\x1f\x7f]|^-?\d)|^-$|[^\x80-\uFFFF\w-]/g,fcssescape=function(ch,asCodePoint){if(asCodePoint){ if(ch==="\0"){return"\uFFFD";} 
return ch.slice(0,-1)+"\\"+ch.charCodeAt(ch.length-1).toString(16)+" ";} 
return"\\"+ch;},
 
unloadHandler=function(){setDocument();},disabledAncestor=addCombinator(function(elem){return elem.disabled===true;},{dir:"parentNode",next:"legend"});try{push.apply((arr=slice.call(preferredDoc.childNodes)),preferredDoc.childNodes);
...
return function(elem){ return"label"in elem&&elem.disabled===disabled||"form"in elem&&elem.disabled===disabled||"form"in elem&&elem.disabled===false&&( elem.isDisabled===disabled|| elem.isDisabled!==!disabled&&("label"in elem||!disabledAncestor(elem))!==disabled);};}
...
function adoptValue(value,resolve,reject){var method;try{ if(value&&jQuery.isFunction((method=value.promise))){method.call(value).done(resolve).fail(reject);}else if(value&&jQuery.isFunction((method=value.then))){method.call(value,resolve,reject);}else{
...
var rbrace=/^(?:\{[\w\W]*\}|\[[\w\W]*\])$/,rmultiDash=/[A-Z]/g;function dataAttr(elem,key,data){var name;
 if(data===undefined&&elem.nodeType===1){name="data-"+key.replace(rmultiDash,"-$&").toLowerCase();data=elem.getAttribute(name);if(typeof data==="string"){try{data=data==="true"?true:data==="false"?false:data==="null"?null:+data+""===data?+data:rbrace.test(data)?JSON.parse(data):data;}catch(e){} 
...
rnoInnerhtml=/<script|<style|<link/i, rchecked=/checked\s*(?:[^=]|=\s*.checked.)/i,rscriptTypeMasked=/^true\/(.*)/,rcleanScript=/^\s*<!(?:\[CDATA\[|--)|(?:\]\]|--)>\s*$/g;function manipulationTarget(elem,content){if(jQuery.nodeName(elem,"table")&&jQuery.nodeName(content.nodeType!==11?content:content.firstChild,"tr")){return elem.getElementsByTagName("tbody")[0]||elem;}
return elem;}
...
return val;}
function getWidthOrHeight(elem,name,extra){ var val,valueIsBorderBox=true,styles=getStyles(elem),isBorderBox=jQuery.css(elem,"boxSizing",false,styles)==="border-box";
...
return value;}}}},removeAttr:function(elem,value){var name,i=0,attrNames=value&&value.match(rnotwhite);if(attrNames&&elem.nodeType===1){while((name=attrNames[i++])){elem.removeAttribute(name);}}}});boolHook={set:function(elem,value,name){if(value===false){ jQuery.removeAttr(elem,name);}else{elem.setAttribute(name,name);}
...
jQuery.trim(jQuery.text(elem)).replace(rspaces," ");}},select:{get:function(elem){var value,option,options=elem.options,index=elem.selectedIndex,one=elem.type==="select-one",values=one?null:[],max=one?index+1:options.length,i=index<0?max:one?index:0; for(;i<max;i++){option=options[i];
if((option.selected||i===index)&&!option.disabled&&(!option.parentNode.disabled||!jQuery.nodeName(option.parentNode,"optgroup"))){ value=jQuery(option).val(); if(one){return value;} 
values.push(value);}}
...
encodeURIComponent(value==null?"":value);};if(jQuery.isArray(a)||(a.jquery&&!jQuery.isPlainObject(a))){ jQuery.each(a,function(){add(this.name,this.value);});}else{
...
s.url=((url||s.url||location.href)+"").replace(rprotocol,location.protocol+"//"); s.type=options.method||options.type||s.method||s.type; s.dataTypes=(s.dataType||"*").toLowerCase().match(rnotwhite)||[""];if(s.crossDomain==null){urlAnchor=document.createElement("a");
...
if(s.cache===false){cacheURL=cacheURL.replace(rts,"");uncached=(rquery.test(cacheURL)?"&":"?")+"_="+(nonce++)+uncached;}
...
return value===undefined? jQuery.css(elem,type,extra): jQuery.style(elem,type,value,extra);},type,chainable?margin:undefined,chainable);};});});jQuery.fn.extend({bind:function(types,data,fn){return this.on(types,null,data,fn);},unbind:function(types,fn){return this.off(types,null,fn);},delegate:function(selector,types,data,fn){return this.on(types,selector,data,fn);},undelegate:function(selector,types,fn){return arguments.length===1?this.off(selector,"**"):this.off(types,selector||"**",fn);}});jQuery.parseJSON=JSON.parse;
...
return jQuery;});jQuery.easing['jswing']=jQuery.easing['swing'];jQuery.extend(jQuery.easing,{def:'easeOutQuad',swing:function(x,t,b,c,d){return jQuery.easing[jQuery.easing.def](x,t,b,c,d);},easeInQuad:function(x,t,b,c,d){return c*(t/=d)*t+b;},easeOutQuad:function(x,t,b,c,d){return-c*(t/=d)*(t-2)+b;},easeInOutQuad:function(x,t,b,c,d){if((t/=d/2)<1)return c/2*t*t+b;return-c/2*((--t)*(t-2)-1)+b;},easeInCubic:function(x,t,b,c,d){return c*(t/=d)*t*t+b;},easeOutCubic:function(x,t,b,c,d){return c*((t=t/d-1)*t*t+1)+b;},easeInOutCubic:function(x,t,b,c,d){if((t/=d/2)<1)return c/2*t*t*t+b;return c/2*((t-=2)*t*t+2)+b;},easeInQuart:function(x,t,b,c,d){return c*(t/=d)*t*t*t+b;},easeOutQuart:function(x,t,b,c,d){return-c*((t=t/d-1)*t*t*t-1)+b;},easeInOutQuart:function(x,t,b,c,d){if((t/=d/2)<1)return c/2*t*t*t*t+b;return-c/2*((t-=2)*t*t*t-2)+b;},easeInQuint:function(x,t,b,c,d){return c*(t/=d)*t*t*t*t+b;},easeOutQuint:function(x,t,b,c,d){return c*((t=t/d-1)*t*t*t*t+1)+b;},easeInOutQuint:function(x,t,b,c,d){if((t/=d/2)<1)return c/2*t*t*t*t*t+b;return c/2*((t-=2)*t*t*t*t+2)+b;},easeInSine:function(x,t,b,c,d){return-c*Math.cos(t/d*(Math.PI/2))+c+b;},easeOutSine:function(x,t,b,c,d){return c*Math.sin(t/d*(Math.PI/2))+b;},easeInOutSine:function(x,t,b,c,d){return-c/2*(Math.cos(Math.PI*t/d)-1)+b;},easeInExpo:function(x,t,b,c,d){return(t==0)?b:c*Math.pow(2,10*(t/d-1))+b;},easeOutExpo:function(x,t,b,c,d){return(t==d)?b+c:c*(-Math.pow(2,-10*t/d)+1)+b;},easeInOutExpo:function(x,t,b,c,d){if(t==0)return b;if(t==d)return b+c;if((t/=d/2)<1)return c/2*Math.pow(2,10*(t-1))+b;return c/2*(-Math.pow(2,-10*--t)+2)+b;},easeInCirc:function(x,t,b,c,d){return-c*(Math.sqrt(1-(t/=d)*t)-1)+b;},easeOutCirc:function(x,t,b,c,d){return c*Math.sqrt(1-(t=t/d-1)*t)+b;},easeInOutCirc:function(x,t,b,c,d){if((t/=d/2)<1)return-c/2*(Math.sqrt(1-t*t)-1)+b;return c/2*(Math.sqrt(1-(t-=2)*t)+1)+b;},easeInElastic:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d)==1)return b+c;if(!p)p=d*.3;if(a<Math.abs(c)){a=c;var s=p/4;}
else var s=p/(2*Math.PI)*Math.asin(c/a);return-(a*Math.pow(2,10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p))+b;},easeOutElastic:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d)==1)return b+c;if(!p)p=d*.3;if(a<Math.abs(c)){a=c;var s=p/4;}
else var s=p/(2*Math.PI)*Math.asin(c/a);return a*Math.pow(2,-10*t)*Math.sin((t*d-s)*(2*Math.PI)/p)+c+b;},easeInOutElastic:function(x,t,b,c,d){var s=1.70158;var p=0;var a=c;if(t==0)return b;if((t/=d/2)==2)return b+c;if(!p)p=d*(.3*1.5);if(a<Math.abs(c)){a=c;var s=p/4;}
else var s=p/(2*Math.PI)*Math.asin(c/a);if(t<1)return-.5*(a*Math.pow(2,10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p))+b;return a*Math.pow(2,-10*(t-=1))*Math.sin((t*d-s)*(2*Math.PI)/p)*.5+c+b;},easeInBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;return c*(t/=d)*t*((s+1)*t-s)+b;},easeOutBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;return c*((t=t/d-1)*t*((s+1)*t+s)+1)+b;},easeInOutBack:function(x,t,b,c,d,s){if(s==undefined)s=1.70158;if((t/=d/2)<1)return c/2*(t*t*(((s*=(1.525))+1)*t-s))+b;return c/2*((t-=2)*t*(((s*=(1.525))+1)*t+s)+2)+b;},easeInBounce:function(x,t,b,c,d){return c-jQuery.easing.easeOutBounce(x,d-t,0,c,d)+b;},easeOutBounce:function(x,t,b,c,d){if((t/=d)<(1/2.75)){return c*(7.5625*t*t)+b;}else if(t<(2/2.75)){return c*(7.5625*(t-=(1.5/2.75))*t+.75)+b;}else if(t<(2.5/2.75)){return c*(7.5625*(t-=(2.25/2.75))*t+.9375)+b;}else{return c*(7.5625*(t-=(2.625/2.75))*t+.984375)+b;}},easeInOutBounce:function(x,t,b,c,d){if(t<d/2)return jQuery.easing.easeInBounce(x,t*2,0,c,d)*.5+b;return jQuery.easing.easeOutBounce(x,t*2-d,0,c,d)*.5+c*.5+b;}});(function(global,factory){  if(typeof define=='function'&&define.amd){ define('ev-emitter/ev-emitter',factory);}else if(typeof module=='object'&&module.exports){ module.exports=factory();}else{ global.EvEmitter=factory();}}(typeof window!='undefined'?window:this,function(){function EvEmitter(){}
var proto=EvEmitter.prototype;proto.on=function(eventName,listener){if(!eventName||!listener){return;} 
var events=this._events=this._events||{}; var listeners=events[eventName]=events[eventName]||[]; if(listeners.indexOf(listener)==-1){listeners.push(listener);}
return this;};proto.once=function(eventName,listener){if(!eventName||!listener){return;} 
this.on(eventName,listener);
 var onceEvents=this._onceEvents=this._onceEvents||{}; var onceListeners=onceEvents[eventName]=onceEvents[eventName]||{}; onceListeners[listener]=true;return this;};proto.off=function(eventName,listener){var listeners=this._events&&this._events[eventName];if(!listeners||!listeners.length){return;}
var index=listeners.indexOf(listener);if(index!=-1){listeners.splice(index,1);}
return this;};proto.emitEvent=function(eventName,args){var listeners=this._events&&this._events[eventName];if(!listeners||!listeners.length){return;}
var i=0;var listener=listeners[i];args=args||[]; var onceListeners=this._onceEvents&&this._onceEvents[eventName];while(listener){var isOnce=onceListeners&&onceListeners[listener];if(isOnce){
...
$.data(this,'plugin_'+readmore,new Readmore(this,options));});}else if(typeof options==='string'&&options[0]!=='_'&&options!=='init'){return this.each(function(){var instance=$.data(this,'plugin_'+readmore);if(instance instanceof Readmore&&typeof instance[options]==='function'){instance[options].apply(instance,Array.prototype.slice.call(args,1));}});}}})(jQuery);function defineSequence(imagesLoaded,Hammer){'use strict';var instances=[],instance=0;var Sequence=(function(element,options){var instanceId=element.getAttribute("data-seq-enabled"); if(instanceId!==null){return instances[instanceId];} 
element.setAttribute("data-seq-enabled",instance);instance++; var defaults={ startingStepId:1,startingStepAnimatesIn:false,cycle:true,
phaseThreshold:true,reverseWhenNavigatingBackwards:false,reverseTimingFunctionWhenNavigatingBackwards:false,moveActiveStepToTop:true,animateCanvas:true, animateCanvasDuration:250,


 autoPlay:false, autoPlayInterval:5000, autoPlayDelay:null, autoPlayDirection:1,

autoPlayButton:true, autoPlayPauseOnHover:true,
 navigationSkip:true, navigationSkipThreshold:250, fadeStepWhenSkipped:true, fadeStepTime:500,
...
return a;}
function getStyle(element,property){var value; if(element.currentStyle){value=element.currentStyle[property];}
else if(document.defaultView&&document.defaultView.getComputedStyle){value=document.defaultView.getComputedStyle(element,"")[property];}
return value;}
function addEvent(element,eventName,handler){if(element.addEventListener){element.addEventListener(eventName,handler,false);return handler;}
else if(element.attachEvent){ var handlerr=function(){handler.call(element);};element.attachEvent("on"+eventName,handlerr);return handlerr;}}
function removeEvent(element,eventName,handler){if(element.addEventListener){element.removeEventListener(eventName,handler,false);}
...
function addClass(elements,name){var element,elementsLength,i;
 if(isArray(elements)===false){elementsLength=1;elements=[elements];}
...
function removeClass(elements,name){var element,elementsLength,i;
 if(isArray(elements)===false){elementsLength=1;elements=[elements];}
else{elementsLength=elements.length;}
...
function addFeatureSupportClasses($el,Modernizr){
 ...
 var prefix="seq-",support="no-touch";if(Modernizr.touch===true){support="touch";}
...
elementsLength=elements.length;
 ...
 for(i=0;i<elementsLength;i++){element=elements[i];rel=element.getAttribute("rel");if(rel===null||rel===self.$container.getAttribute("id")){relatedElements.push(element);}}
return relatedElements;},show:function(element,duration){if(self.propertySupport.transitions===true){element.style[Modernizr.prefixed("transitionDuration")]=duration+"ms";element.style[Modernizr.prefixed("transitionProperty")]="opacity, "+Modernizr.prefixed("transform");element.style.opacity=1;}
else{self.animationFallback.animate(element,"opacity","",0,1,duration);}},hide:function(element,duration,callback){if(self.propertySupport.transitions===true){element.style[Modernizr.prefixed("transitionDuration")]=duration+"ms";element.style[Modernizr.prefixed("transitionProperty")]="opacity, "+Modernizr.prefixed("transform");element.style.opacity=0;}
else{self.animationFallback.animate(element,"opacity","",1,0,duration);}
...
var options=self.options;delay=this.getDelay(delay,options.autoPlayDelay,options.autoPlayInterval);
if(continuing===undefined){self.started(self);}
...
self.canvas.getTransformProperties();},getSteps:function(canvas){var steps=[],stepId,step,stepElements=canvas.children,stepsLength=stepElements.length,i; self.stepProperties={};for(i=0;i<stepsLength;i++){step=stepElements[i];stepId=i+1;steps.push(step);
 self.stepProperties[stepId]={};self.stepProperties[stepId].isActive=false;}
return steps;},getTransformProperties:function(){var i,step,stepId,canvasTransform;for(i=0;i<self.noOfSteps;i++){step=self.$steps[i];stepId=i+1;canvasTransform={"seqX":0,"seqY":0,"seqZ":0};
 canvasTransform.seqX+=step.offsetLeft*-1;canvasTransform.seqY+=step.offsetTop*-1;self.stepProperties[stepId].canvasTransform=canvasTransform;}},move:function(id,animate){if(self.options.animateCanvas===true){ var duration=0,transforms;if(animate===true&&self.firstRun===false){duration=self.options.animateCanvasDuration;} 
...
var maxDuration=Math.max.apply(Math,durations),maxDelay=Math.max.apply(Math,delays),maxTotal=Math.max.apply(Math,lengths);var watchedMaxDuration=Math.max.apply(Math,watchedDurations),watchedMaxDelay=Math.max.apply(Math,watchedDelays),watchedMaxTotal=Math.max.apply(Math,watchedLengths);return{stepId:stepId,stepElement:stepElement,children:stepChildren,animatedChildren:stepAnimatedChildren,watchedTimings:{maxDuration:watchedMaxDuration,maxDelay:watchedMaxDelay,maxTotal:watchedMaxTotal},timings:{maxDuration:maxDuration,maxDelay:maxDelay,maxTotal:maxTotal}};},getPhaseThreshold:function(ignorePhaseThreshold,phaseThresholdOption,isAnimating,currentPhaseDuration){var phaseThresholdTime=0;
 if(isAnimating===true&&self.options.ignorePhaseThresholdWhenSkipped===true){ignorePhaseThreshold=true;}
if(ignorePhaseThreshold===undefined){if(phaseThresholdOption===true){
 phaseThresholdTime=currentPhaseDuration;}else if(phaseThresholdOption!==false){ phaseThresholdTime=phaseThresholdOption;}}
return phaseThresholdTime;},getReversePhaseDelay:function(currentPhaseTotal,nextPhaseTotal,phaseThresholdOption,ignorePhaseThresholdWhenSkippedOption,isAnimating){var phaseDifference=0,current=0,next=0;

 if(phaseThresholdOption!==true&&(ignorePhaseThresholdWhenSkippedOption===false||isAnimating===false)){phaseDifference=currentPhaseTotal-nextPhaseTotal;if(phaseDifference>0){next=phaseDifference;}else if(phaseDifference<0){current=Math.abs(phaseDifference);}}
...
var i,stepProperties,stepElement,stepId,phaseSkipped; self.ui.show(nextStepElement,0);if(self.options.navigationSkip===true){ self.navigationSkipThresholdActive=true;if(self.phasesAnimating!==0){ clearTimeout(self.phaseThresholdTimer);clearTimeout(self.nextPhaseStartedTimer);
 if(self.options.fadeStepWhenSkipped===true){
for(i=1;i<=self.noOfSteps;i++){stepProperties=self.stepProperties[i]; if(stepProperties.isActive===true&&i!==id){stepElement=self.$steps[i-1];stepId=i;phaseSkipped={};phaseSkipped.id=stepId;phaseSkipped.element=stepElement; self.phasesSkipped.push(phaseSkipped); self.animation.stepSkipped(stepElement);}}}}
 
self.navigationSkipThresholdTimer=setTimeout(function(){self.navigationSkipThresholdActive=false;},self.options.navigationSkipThreshold);}},stepSkipped:function(stepElement){


 self.ui.hide(stepElement,self.options.fadeStepTime,function(){});},changeStep:function(id){ var stepToAdd="seq-step"+id; if(self.currentStepId!==undefined){var stepToRemove="seq-step"+self.currentStepId;addClass(self.$container,stepToAdd);removeClass(self.$container,stepToRemove);}else{addClass(self.$container,stepToAdd);}},reverseProperties:function(phaseProperties,phaseDelay,phaseThresholdTime,ignorePhaseThreshold,options){var animation=this,phaseElements=phaseProperties.children,noOfPhaseElements=phaseElements.length,stepDurations=phaseProperties.timings,el,i,timingFunction='',timingFunctionReversed='',duration,delay,total,maxTotal,maxWatchedTotal,totals=[],watchedTotals=[];for(i=0;i<noOfPhaseElements;i++){el=phaseElements[i]; duration=convertTimeToMs(getStyle(el,Modernizr.prefixed("transitionDuration")));delay=convertTimeToMs(getStyle(el,Modernizr.prefixed("transitionDelay"))); total=duration+delay;
...
if(options.reverseTimingFunctionWhenNavigatingBackwards===true){timingFunction=getStyle(el,Modernizr.prefixed("transitionTimingFunction"));timingFunctionReversed=animation.reverseTimingFunction(timingFunction);} 
...
reversePhaseDelay=animation.getReversePhaseDelay(currentPhaseProperties.timings.maxTotal,nextPhaseProperties.timings.maxTotal,self.options.phaseThreshold,self.options.ignorePhaseThresholdWhenSkipped,self.isAnimating);
 currentPhaseTotal=animation.reverseProperties(currentPhaseProperties,reversePhaseDelay.current,0,ignorePhaseThreshold,self.options);nextPhaseTotal=animation.reverseProperties(nextPhaseProperties,reversePhaseDelay.next,phaseThresholdTime,ignorePhaseThreshold,self.options);animation.startAnimateOut(self.currentStepId,currentStepElement,-1,currentPhaseTotal); self.isAnimating=true;if(self.firstRun===false){ animation.currentPhaseStarted(self.currentStepId);}
...
if(self.firstRun===false||self.options.startingStepAnimatesIn===true){self.stepProperties[id].isActive=true; self.nextPhaseStartedTimer=setTimeout(function(){animation.nextPhaseStarted(id,hashTagNav);},phaseThresholdTime); self.phaseThresholdTimer=setTimeout(function(){ addClass(nextStepElement,"seq-in");removeClass(nextStepElement,"seq-out");
 if(nextPhaseTotal===undefined){nextPhaseProperties=self.animation.getPhaseProperties(id,"next");nextPhaseTotal=nextPhaseProperties.watchedTimings.maxTotal;} 
...
self.firstRun=false;},startAnimateOut:function(id,currentStepElement,direction,currentPhaseTotal){var animation=this,currentPhaseProperties;if(direction===1){
removeClass(currentStepElement,"seq-in");addClass(currentStepElement,"seq-out");
 currentPhaseProperties=animation.getPhaseProperties(id,"current");currentPhaseTotal=currentPhaseProperties.watchedTimings.maxTotal;}else{removeClass(currentStepElement,"seq-in");}
...
var timingFunctionToCubicBezier={"linear":"cubic-bezier(0.0,0.0,1.0,1.0)","ease":"cubic-bezier(0.25, 0.1, 0.25, 1.0)","ease-in":"cubic-bezier(0.42, 0.0, 1.0, 1.0)","ease-in-out":"cubic-bezier(0.42, 0.0, 0.58, 1.0)","ease-out":"cubic-bezier(0.0, 0.0, 0.58, 1.0)"};var cubicBezier,cubicBezierLength,reversedCubicBezier,i; if(timingFunction.indexOf("cubic-bezier")<0){

 timingFunction=timingFunction.split(",")[0]; timingFunction=timingFunctionToCubicBezier[timingFunction];} 
...
if(self.firstRun===false){this.moveCanvas(nextStepElement,currentStepElement,direction,true); self.isAnimating=true; self.animationStarted(self.currentStepId,self); self.stepEndedTimer=setTimeout(function(){self.animation.stepEnded(self.currentStepId);},self.options.fallback.speed);} 
...
return paginationLinks;},update:function(){if(self.$pagination.elements!==undefined){var i,j,id=self.currentStepId-1,currentPaginationLink,currentPaginationLinksLength,paginationLength=self.$pagination.elements.length;
 if(self.$pagination.currentLinks!==undefined){currentPaginationLinksLength=self.$pagination.currentLinks.length;for(i=0;i<currentPaginationLinksLength;i++){currentPaginationLink=self.$pagination.currentLinks[i];removeClass(currentPaginationLink,"seq-current");}} 
self.$pagination.currentLinks=[];
 for(j=0;j<paginationLength;j++){currentPaginationLink=self.$pagination.links[j][id];self.$pagination.currentLinks.push(currentPaginationLink);addClass(currentPaginationLink,"seq-current");}}
return self.$pagination.currentLinks;}};self.hashTags={init:function(id){if(self.options.hashTags===true){var correspondingStepId,newHashTag;self.hasPushstate=!!(window.history&&history.pushState); newHashTag=location.hash.replace("#!",""); self.stepHashTags=this.getStepHashTags(); if(newHashTag!==""){ self.currentHashTag=newHashTag;correspondingStepId=this.hasCorrespondingStep(self.currentHashTag);

 if(correspondingStepId>-1){id=correspondingStepId+1;}}}
...
for(i=0;i<elementsLength;i++){element=elements[i];rel=element.getAttribute("rel");
if(rel===self.$container.id&&element.getAttribute("data-seq-button")===null){element.setAttribute("data-seq-button",true);buttonEvent(element,rel);} 
...
self.isMouseOver=false;}); self.manageEvents.list.mouseleave.push({"element":self.$container,"handler":handler});return null;},swipeNavigation:function(){
if(window.addEventListener===undefined){self.hammerTime=false;return;}
...
hammerDirection=getHammerDirection(self.options.swipeEvents);self.hammerTime.get("swipe").set({direction:hammerDirection});self.manageEvents.list.hammer.push({"element":self.$container,"handler":handler});}},keyNavigation:function(){var handler=addEvent(document,"keyup",function(event){if(!event){event=window.event;} 
...
switch(event.keyCode){case 37:self.options.keyEvents.left(self);break;case 39:self.options.keyEvents.right(self);break;}});self.manageEvents.list.keyup.push({"element":document,"handler":handler});},resizeThrottle:function(){ function throttledEvents(){
 setTimeout(function(){self.canvas.getTransformProperties();if(self.propertySupport.transitions===true){self.canvas.move(self.currentStepId,false);}},500); self.throttledResize(self);}
var throttleTimer,handler;handler=addEvent(window,"resize",function(e){clearTimeout(throttleTimer);throttleTimer=setTimeout(throttledEvents,resizeThreshold);});self.manageEvents.list.resize.push({"element":window,"handler":handler});},pageVisibility:function(){var handler=addEvent(document,visibilityChange,function(){if(document[hidden]){self.autoPlay.pause();}else{self.autoPlay.unpause();}},false);self.manageEvents.list[visibilityChange].push({"element":document,"handler":handler});}}};self.init=function(element){var id,prevStep,prevStepId,transformProperties,goToFirstStep; self.options=extend(defaults,options);self.isAnimating=false;self.isReady=false; self.$container=element;self.$screen=self.$container.querySelectorAll(".seq-screen")[0];self.$canvas=self.$container.querySelectorAll(".seq-canvas")[0];self.$steps=self.canvas.getSteps(self.$canvas); self.noOfSteps=self.$steps.length;self.phasesAnimating=0;self.phasesSkipped=[]; addFeatureSupportClasses(self.$container,Modernizr); id=self.options.startingStepId;addClass(self.$container,"seq-active");
 self.propertySupport=self.animation.getPropertySupport();self.isFallbackMode=self.animation.requiresFallbackMode(self.propertySupport); self.canvas.init(id); self.canvas.removeNoJsClass(self); self.manageEvents.init(); self.autoPlay.init(); self.firstRun=true;
 id=self.hashTags.init(id); if(self.options.autoPlayDirection===1){prevStepId=id-1;self.prevStepId=(prevStepId<1)?self.noOfSteps:prevStepId;}else{prevStepId=id+1;self.prevStepId=(prevStepId>self.noOfSteps)?1:prevStepId;} 
...
currentStepElement=self.$steps[self.currentStepId-1];nextStepElement=self.$steps[id-1];self.animation.moveActiveStepToTop(currentStepElement,nextStepElement); self.animation.changeStep(id);if(self.isFallbackMode===false){
 self.animation.resetInheritedSpeed(id);if(self.firstRun===false||(self.firstRun===true&&self.options.startingStepAnimatesIn===true)){ self.animationStarted(id,self);} 
self.canvas.move(id,true);
 self.animation.manageNavigationSkip(id,nextStepElement);if(direction===1){self.animation.forward(id,currentStepElement,nextStepElement,ignorePhaseThreshold,hashTagNav);}else{self.animation.reverse(id,currentStepElement,nextStepElement,ignorePhaseThreshold,hashTagNav);}} 
...
item.img=$(img).on('load.mfploader',onLoadComplete).on('error.mfploader',onLoadError);img.src=item.src;
...
item.preloaded=true;}}});var RETINA_NS='retina';$.magnificPopup.registerModule(RETINA_NS,{options:{replaceSrc:function(item){return item.src.replace(/\.\w+$/,function(m){return'@2x'+m;});},ratio:1
},proto:{initRetina:function(){if(window.devicePixelRatio>1){var st=mfp.st.retina,ratio=st.ratio;ratio=!isNaN(ratio)?ratio:ratio();if(ratio>1){_mfpOn('ImageHasSize'+'.'+RETINA_NS,function(e,item){item.img.css({'max-width':item.img[0].naturalWidth/ratio,'width':'100%'});});_mfpOn('ElementParse'+'.'+RETINA_NS,function(e,item){item.src=st.replaceSrc(item,ratio);});}}}}});_checkInstance();}));
